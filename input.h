...
//Require string Input is not null
string inputRequiredString(string prompt, bool required)
{
	string input;
	...
	do {
		...
		{
			cout << "ERROR: Input value must NOT be EMPTY\n";
	...
	do
	{
		...
		if (!(cin >> input))
		{
			...
			//cout << "ERROR: Invalid input. Must be NOT EMPTY\n";
			cin.clear();
			cin.ignore(999, '\n');
		}
		else
...
//PreCondition: valid string option
//PostCondition: returns an uppercase  of the option
...
double inputFloat(string prompt, bool posNeg)
{
	float input;
	do
	{
		...
		if (!(cin >> input))
		{
			cout << "ERROR: Invalid input. Must be a FLOAT type.\n";
			cin.clear();
			cin.ignore(999, '\n');
		}
		else if (posNeg && input <= 0.0)
